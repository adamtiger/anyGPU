...
__global__ void cu_tensor_mul_tt_kernel_f32(const int length, const float32* dlhs, const float32* drhs, float32* dout)
{
	int tid = blockDim.x * blockIdx.x + threadIdx.x;
	...
	const Tensor<float32, CUDA>& out)
{
	...
	cu_tensor_mul_tt_kernel_f32<<<kpms.grid_size, kpms.block_size>>>(length, dlhs, drhs, dout);
	CUDA_CHECK_LAST_ERROR();
}
	...
	float32* dy)
{
	...
	__shared__ float x_shared[TH * TK];
	__shared__ float w_shared[TK * TW];
	...
	for (int tk = 0; tk < num_tiles; ++tk)
	{
		...
		{
			float* sx = x_shared + shrd_offset_x + tidx * RK + i * TK;
			const float* gx = dx + glob_offset_x + tidx * RK + i * x_width;
			*reinterpret_cast<float2*>(sx) = *reinterpret_cast<const float2*>(gx);
		}
		
		for (int i = 0; i < ROWS_W; ++i)
		{
			float* sw = w_shared + shrd_offset_w + tidx * RW + i * TW;
		...
		for (int rk = 0; rk < TK / RK; ++rk)
		{
			...
			float* sx = x_shared + lane_idx * RH * TK + rk * RK;
			for (int r = 0; r < RH; ++r)
			{
				*reinterpret_cast<float2*>(x_reg[r]) = *reinterpret_cast<float2*>(sx + r * TK);
			}

			float* sw = w_shared + rk * RK * TW + tidx * RW;
			for (int r = 0; r < RK; ++r)
			{

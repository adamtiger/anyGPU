...
__global__ void cu_mlp_gemma2_dp_linear_f32_v2_kernel(
	const int sl,
	const float32* dx,
	const float32* dwdp,
	float32* dy)
{
	int j0 = blockIdx.x * TS_H;
	...
	for (int tk = 0; tk < NUM_KTILES; ++tk)
	{
		...
		__shared__ float32 shrd_w[TS_H * TS_K];
		for (int r = 0; r < NR_LOAD; ++r)
		{
			int wrow_id = lane_id / 2;
			int glob_index_w = (j0 + wrow_id * NR_LOAD + r) * x_width + tk * TS_K + WARP_SIZE * (lane_id % 2) + threadIdx.x;
			int shrd_index_w = (wrow_id * NR_LOAD + r) * TS_K + WARP_SIZE * (lane_id % 2) + threadIdx.x;
		...
		for (int r = 0; r < NR; ++r)
		{
			for (int k = 0; k < TS_K; ++k)
			{
				...
				int index_w = threadIdx.x * TS_K + k;
	...
	Tensor<float32, CUDA>& yt)
{
	...
	dim3 gs = { calc_req_num_blocks(w_height, TS_H), calc_req_num_blocks(sl, TS_H), 1 };

	cu_mlp_gemma2_dp_linear_f32_v2_kernel<<<gs, bs>>>(sl, dx, dwdp, dy);

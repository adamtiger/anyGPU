...
__global__ void cu_fast_mm_f16_v2_kernel(
	const int x_width,
	const int w_width,
	const float16* dx,
	const float16* dw,
	float16* dy)
{
	...
	for (int tk = 0; tk < NUM_KTILES; ++tk)
	{
		...
		__shared__ float16 shrd_x[TS_H * TS_K];
		for (int r = 0; r < NR; ++r)
		{
		...
		__shared__ float16 shrd_w[TS_K * TS_W];
		for (int r = 0; r < NR; ++r)
		{
			int wrow_id = lane_id / 2;
	...
	Tensor<float16, CUDA>& yt)
{
	...
	cu_fast_mm_f16_v2_kernel<<<gs, bs>>>(x_width, w_width, dx, dw, dy);

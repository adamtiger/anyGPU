...
__global__ void cu_fast_mm_f32_v4_2_kernel(
	const int x_common,
	const int w_width,
	const float32* dx,
	const float32* dw,
	float32* dy)
{
	...
	__shared__ float x_shared[2][TK * TH];
	...
	// x_width
	int x_width = TH * gridDim.y;

	// load the first tiles from glob to shared
	// load data from global to shared
	int glob_offset_x = 0 * TK * x_width + lane_idx * x_width * ROWS_X / 2 + bh * TH;//bh * TH * x_width + 0 * TK + lane_idx * ROWS_X * x_width;
	int glob_offset_w = 0 * TK * w_width + bw * TW + lane_idx * ROWS_W * w_width;

	int shrd_offset_x = lane_idx * ROWS_X / 2 * TH;
	int shrd_offset_w = lane_idx * ROWS_W * TW;

	for (int i = 0; i < ROWS_X / 2; ++i)  // coalesced read (load.64)
	{
		float* sx = x_shared[0] + shrd_offset_x + tidx * 2 + i * TH;
		const float* gx = dx + glob_offset_x + tidx * 2 + i * x_width;
		*reinterpret_cast<float2*>(sx) = *reinterpret_cast<const float2*>(gx);
	}
	...
	for (int tk = 1; tk < num_tiles; ++tk)
	{
		...
		// load data from global to shared
		int glob_offset_x = tk * TK * x_width + lane_idx * x_width * ROWS_X / 2 + bh * TH;//bh * TH * x_width + tk * TK + lane_idx * ROWS_X * x_width;
		...
		for (int i = 0; i < ROWS_X / 2; ++i)  // coalesced read (load.64)
		{
			float* sx = x_shared[mem_load_ix] + shrd_offset_x + tidx * 2 + i * TH;
			const float* gx = dx + glob_offset_x + tidx * 2 + i * x_width; //dx + glob_offset_x + tidx * RK / 2 + i * x_width;
			*reinterpret_cast<float2*>(sx) = *reinterpret_cast<const float2*>(gx);
		}
		...
		for (int rk = 0; rk < TK / RK; ++rk)
		{
			...
			float* sx = x_shared[comp_ix] + lane_idx * RH + rk * RK * TH;
			for (int r = 0; r < RH; ++r)
			{
				x_reg[r][0] = *(sx + r);
				x_reg[r][1] = *(sx + r + TH);
	...
	{
		// load from shared to registers
		float* sx = x_shared[1] + lane_idx * RH + rk * RK * TH;
		for (int r = 0; r < RH; ++r)
		{
			x_reg[r][0] = *(sx + r);
			x_reg[r][1] = *(sx + r + TH);
	...
	Tensor<float32, CUDA>& yt)
{
	...
	dim3 gs = { calc_req_num_blocks(w_width, TW), calc_req_num_blocks(x_width, TH), 1 };

	cu_fast_mm_f32_v4_2_kernel<<<gs, bs>>>(x_common, w_width, dx, dw, dy);

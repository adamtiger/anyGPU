...
struct GemmaModelweights
{
	...
	void load_weights_from_safetensors(const std::string& weights_folder_path)
	{
		...
		_set_weights_from_named_tensors(loaded_tensors);
	}
	...
	void load_weights_from_datfiles(const std::string& weights_folder_path)
	{
		...
		std::vector<Tensor<dtype, CUDA>> loaded_tensors;
		//_load_tensors_from_safetensors(weights_folder_path, loaded_tensors);
		_load_tensors_from_datfiles(weights_folder_path, loaded_tensors);
...
private:
		...
		std::vector<Tensor<dtype, CUDA>>& loaded_tensors)
	{
		...
		const std::regex dat_extension("\\.dat$");

		for (auto const& entry : std::filesystem::directory_iterator{ weights_folder_path })
		{
			if (entry.is_regular_file())
			{
				auto weight_path = entry.path().string();
				auto file_name = entry.path().filename().string();
				if (std::regex_search(file_name, dat_extension))
				{
					auto tensor = load_w(weight_path);
					tensor.name = file_name.substr(0, file_name.size() - 4);  // 4: length of '.dat'
					loaded_tensors.push_back(tensor);
				}
			}
		}
	}

	void _set_weights_from_named_tensors(std::vector<Tensor<dtype, CUDA>>& loaded_tensors)
	{
		// identify and assign the tensors
